...
\newenvironment{problem}%	окружение для оформления примера
	{\par \vspace{\baselineskip} \refstepcounter{problem} \begin{flushright} \textit{Пример \theproblem} \end{flushright} \vspace{-\baselineskip} \begin{oframed}}%	это выполняется при отрытии окружения
...
\begin{tikzpicture}	
	...
	\fill (LineSeparator1_2) circle (1pt);	
	\node[point] (LineSeparator1_3) [above right = 0.25ex of LineSeparator1_2]{$x$}; 	
	
	
	\node[point] (LineSeparator1_3) [right = 7.5ex of block1_3]{}; 		
	\draw (LineSeparator1_2) |- (LineSeparator1_3) [vec] to (block1_3);
	
	
	%Problem is here
	\node[point] (LineSeparator1_4) [left = 15ex of block1_3]{}; 	
	\draw (block1_3) |- (LineSeparator1_4) [vec] to (sum1_1);
	%Problem is here
		
	\node[block] (block1_4)[right = 15ex of block1_2] {$\mathbb{C}$};	
	\draw[vec] (block1_2) to (block1_4);
	
	\node[point] (LineSeparator1_5) [right = 25ex of block1_4]{$y$}; 
	...
	%Bottom row
	\node[block] (block2_1) [below = 21.25ex of input_int] {$ B $};
	\node[point] (LineSeparator1_1) 	 [left = 2.5ex of block1_1] {};
