...
\bibindex{УДК 681.513.5\\ББК \pbox[lt]{14.2.6\\Н561}}
\copyrighttable{ }
{	\item С.\=,А.~Нестеров, \theyear \\
	\item Санкт-Петербургский государственный\\ политехнический университет, \theyear }
\begin{abstract}
\fulltitle{Нестеров С.\=,А.}{\textbf{Системы оптимального управления}\=,: конспект лекций --- СПб.\=,: Изд-во политехнического ун-та, \theyear . --- 73~с.}
...
\begin{problem} \label{ex:dyn_prog:third_example}
...
\item Область \textbf{1} $ \left\{ \left( \begin{matrix}
	x_1 \\
	x_2
\end{matrix} \right) \=, : \enskip \frac{\partial R}{\partial x_2} > 0 \right\} \enskip \Rightarrow \enskip \vspace{1ex} u^* = -1 \enskip \Rightarrow \enskip $ уравнение Беллмана принимает вид: 
...
\begin{itemize}
\item в первой:
\[ x_1 + \frac{x_2^2}{2} - \frac{ { \bigl( T^* - x_2 \bigr) }^2 }{4} = 0 \] \pagebreak
\item во второй:
...
Таким образом, можно изобразить семейство изохрон, и для любой точки, например, $ \left( \begin{mmatrix} x_1 \\ x_2 \end{mmatrix} \right) = \left( \begin{mmatrix} 4 \\ 0 \end{mmatrix} \right) $ изобразить вектор $ grad \=, R = \left( \begin{mmatrix} 0.5 \\ 1 \end{mmatrix} \right) $, а вектор фазовой скорости $ \Phi (x,u) = \left( \begin{mmatrix} 0 \\ u \end{mmatrix} \right) $. Из всех возможных управлений лишь при $ u = -1 $ будет наибольшая проекция на направление антиградиента к изохроне в этой точке. Значит, оптимальное управление $ u^* \left( \begin{mmatrix} 4 \\ 0 \end{mmatrix} \right) = -1 $.

\end{problem}

\begin{figure}[ht] \centering
\begin{tikzpicture}[xscale=0.35,yscale=0.7]
	\draw[very thin,gray,xstep=4mm,ystep=2mm] (-10,-5) grid (10,5);		% сетка
	\draw[semithick,->] (-10,0) -- (10,0) node[right] {$ x_1 $};
	\draw[semithick,->] (0,-5) -- (0,5) node[above] {$ x_2 $};
	\foreach \x in {-8,-6,-4,-2,2,4,6,8}
		\draw[semithick] (\x cm,1mm) -- (\x cm,-1mm) node[anchor=north] {$\x$};
	\foreach \y in {-4,-3,-2,-1,1,2,3,4}
		\draw[semithick] (2mm,\y cm) -- (-2mm,\y cm) node[anchor=east] {$\y$};	
	
	\foreach \T in {1,2,3,4} {
		\draw[blue,smooth,variable=\y,samples=100,domain=-\T:\T]
			plot({ -\y*\y/2 + (\T-\y)*(\T-\y)/4 },{\y})
			plot({ \y*\y/2 - (\T+\y)*(\T+\y)/4 },{\y});
	}

	\draw[thick,blue,rotate=90] ({-sqrt(20)},-10) parabola bend (0,0) (0,0) parabola ({sqrt(20)},10);

	\begin{scope} \scriptsize
		\node at (0.3,0.25) {$1$};
		\node at (1,0.4) {$2$};
		\node at (1.5,0.75) {$3$};
		\node at (2.5,1.5) {$ T^* \!\! = 4 \, \text{с} $};
	\end{scope}
	
	\draw[semithick,->] (4,0) -- (4.5,1) node[inner sep=0,above right=0mm and 1mm]{$ grad \, T $};
	\draw[dashed] (4,0) -- (3.5,-1);
	\draw[semithick,->] (4,0) -- (4,-1);

	\node at (-8,-4) {$+$};
	\node at (8,4) {$-$};
\end{tikzpicture}
\footnotesize \caption{Решение задачи, рассмотренной в примере~\ref{ex:dyn_prog:third_example} \label{fig:dyn_prog:third_example}}
\end{figure}
...
\end{tikzpicture}
\footnotesize \caption{\centering Наблюдатель Люенбергера полной размерности для объекта в примере~\ref{ex:stochastic:Luenberger:first_example} \label{fig:stochastic:Luenberger:first_example}}
\end{figure}
...
\end{tikzpicture}
\footnotesize \caption{\centering Наблюдатель Люенбергера сокращённой размерности для объекта в примере~\ref{ex:stochastic:Luenberger:second_example} \label{fig:stochastic:Luenberger:second_example}}
\end{figure}
