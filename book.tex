...
\begin{tikzpicture}	
	...
	\node[point] (input) [left = 4ex of block1_1] {$ u $};	% Расстояние в явном виде писать необязательно, здесь можно написать просто [left = of block1_1] и будет нормально. Явно задавать расстояние следует в том случае, когда надо сделать его меньше, чем делается автоматически, например, в случае, когда рисунок не помещается на страницу.
	\draw[vec] (input) to (block1_1);

	\mysum{right = 2.5ex of block1_1}{sum1_1};
	\draw[vec] (block1_1) to (sum1_1);

	\node[block] (block1_2)[right = 20ex of block1_1] {$\in$};	% надо задавать размещение относительно соседних элементов, а не относительно какой-нибудь штуки на другом краю рисунка :) здесь надо написать, что block1_2 справа от sum1_1
	% знак интегрирования на самом блоке можно заменить на 1/p
	\draw[vec] (sum1_1) to (block1_2);
		
	\node[block] (block1_3)[below = 2.5ex of block1_2] {$A$};
