...
\begin{itemize}
\item Приницип стохастической эквивалентности - утверждает возможность замены реальных переменных состояния $X$ в регуляторе их оценками  $\hat{x}$, вычисляемыми по измеряемому выходу $y$; 
...
\end{itemize}
\par Структура наблюдателя полного порядка, очевидно, избыточна, т.к. в ней определяются и те координаты, которые доступны для измерения. Поэтому, в 1968 году Люенбергер Д.Ж. предложил идею построения наблюдателей сокращенной размерности. Пусть в системе:
 \[\left\{ \begin{array}{*{35}{l}}
   \dot{x}=Ax+Bu  \\
   x(0)  \\
   y=Cx  \\
...
\begin{tikzpicture}	
	[	auto,
		...
		vec/.style={->,>=stealth,semithick,inner sep=1ex},
		point/.style={inner sep=0pt}							]
	%%Первая колонка блоков
	\node[block] (block1_1) {$ B $};
		
	\node[point] (input) [left = of block1_1] {$ u $};	% Расстояние в явном виде писать необязательно, здесь можно написать просто [left = of block1_1] и будет нормально. Явно задавать расстояние следует в том случае, когда надо сделать его меньше, чем делается автоматически, например, в случае, когда рисунок не помещается на страницу.
	\draw[vec] (input) to (block1_1);

	\mysum{right = 3ex of block1_1}{sum1_1};
	\draw[vec] (block1_1) to (sum1_1);

	\node[block] (block1_2)[right = 13ex of sum1_1] {$1/p$};	% надо задавать размещение относительно соседних элементов, а не относительно какой-нибудь штуки на другом краю рисунка :) здесь надо написать, что block1_2 справа от sum1_1
	
	\node[point] (inp2) [above = 2ex of block1_2] {$x\left( 0 \right)$};
	\draw[vec] (inp2) to (block1_2);
	
	
	% знак интегрирования на самом блоке можно заменить на 1/p
	\draw[vec] (sum1_1) to (block1_2);
		
	\node[block] (block1_3)[below = of block1_2] {$A$};
	
	\node[block] (block1_4)[right = 20ex of block1_2] {$\mathbb{C}$};


	%\draw (block1_2) [vec] to (block1_4);
	
	%\node[point] (LineSeparator1_2) [right = 7.5ex of block1_2]{};
	
	\draw[vec] (block1_2) to node(LineSeparator1_2){$ \bar{x} $} (block1_4);
	
	\fill (LineSeparator1_2.south) circle (2pt);
	\draw[vec] (LineSeparator1_2) |- (block1_3);
	
	\draw[vec] (block1_3) -| (sum1_1);
	
	\node[point] (output) [right = of block1_4] {$ y $};
	\draw[vec] (block1_4) to (output);


	%%Вторая линия
	\node[block] (block2_1)[below =30ex of block1_1] {$B$};
	\mysum{right = 3ex of block2_1}{sum2_1};
	
	
	\node[block] (block2_2)[right = 13ex of sum2_1] {$1/p$};
	\node[block] (block2_3)[above = of block2_2] {$\left( D-GC \right)\left( A{{L}_{1}}-A{{L}_{2}}G \right)$};
	\node[block] (block2_4)[below = of block2_2] {$\left( D-GC \right)A{{L}_{2}}$};
	\node[point] (LineSeparator1_1) [left = 3.5ex of block1_1] {};
	\fill (LineSeparator1_1) circle (2pt);	
	\draw[vec] (LineSeparator1_1) |- (block2_1);	

	
	\draw[vec] (block2_1) to (sum2_1);	
	
	\draw[vec] (sum2_1) to (block2_2);	
	
	
	\node[block] (block2_5)[right = 20ex of block2_2]{${{L}_{2}}$};
		
	
	\draw[vec] (block2_2) to (block2_5);	
%	\fill (LineSeparator2_1.south) circle (1pt);
	
	\draw[vec] (block2_3) -| (sum2_1);
	\draw[vec] (block2_4) -| (sum2_1);
	
	\node[point] (LineSeparator2_1) [right = 15ex of block2_2]{};
	\fill (LineSeparator2_1) circle (2pt);



	\draw[vec] (LineSeparator2_1) |- (block2_4);	
	

		
	\mysum{right =3ex of block2_5}{sum2_2};	
	
	
	\draw[vec] (block2_5) to (sum2_2);	

	\node[point] (LineSeparator1_3) [above  = 34.7ex of sum2_2]{};

	\fill (LineSeparator1_3.south) circle (2pt);	
	
	
	
	\node[block] (block2_6)[below = 20ex of LineSeparator1_3] {${{L}_{1}}+{{L}_{2}}G$};
	\node[point] (LineSeparator1_4) [right = 15ex of LineSeparator1_3]{};
	
	\draw[vec] (LineSeparator1_3) to (block2_6);
	\draw[vec] (block2_6) to (sum2_2);
	
	\node[point] (output2) [right = 2ex of sum2_2] {$\overline{x}$};	
	
	\draw[vec] (sum2_2) to (output2);

	\node[point] (LineSeparator1_4) [below = 10ex of LineSeparator1_3] {};
	\fill (LineSeparator1_4.south) circle (2pt);	
	%\draw[vec] (LineSeparator1_4) -| (block2_3);
	
	\node[point] (LineSeparator1_5) [right = 6ex of block2_3] {};
	
		
	\draw (LineSeparator1_4) -| (LineSeparator1_5) [vec] to (block2_3);
	
%	\node[point] (LineSeparator1_2) [right = 7.5ex of block1_2]{}; 	% вместо этого лучше сначала нарисовать блок С (который справа от интегратора), нарисовать к нему вектор и на этом векторе поставить узел с текстом $x$, а затем уже от этого узла вести вектор к блоку А. (см. мой рисунок в предыдущем пункте)
%	\fill (LineSeparator1_2) circle (1pt);		% соответственно и точку рисовать как-то так: \fill (x.south) circle (1pt);
%	\node[point] (LineSeparator1_3) [above right = 0.25ex of LineSeparator1_2]{$x$}; 	% ну это тоже будет лишнее
	
	
%	\node[point] (LineSeparator1_3) [right = 7.5ex of block1_3]{}; 		% здесь точку ставить не надо, потому что можно просто написать так: \draw[vec] (x) |- (block1_3);
%	\draw (LineSeparator1_2) |- (LineSeparator1_3) [vec] to (block1_3);	% см. коммент. в пред. строке
	
	
	%Problem is here
%	\node[point] (LineSeparator1_4) [left = 15ex of block1_3]{}; 	% здесь тоже точка не нужна!
%	\draw (block1_3) |- (LineSeparator1_4) [vec] to (sum1_1);		% \draw[vec] (block1_3) |- (sum1_1);
	%Problem is here
		
	%\node[block] (block1_4)[right = 15ex of block1_2] {$\mathbb{C}$};
%	\draw[vec] (block1_2) to (block1_4);	% вот на этом векторе и надо поставить узел x (то, о чём я говорил выше)
	
%	\node[point] (LineSeparator1_5) [right = 25ex of block1_4]{$y$}; % лучше поставить узел на векторе, который ты рисуешь в следующей строке, и написать $y$ на этом узле, чтобы от него можно было сделать ответвление так, как я говорил выше
%	\draw[vec] (block1_4) to (LineSeparator1_5);
	...
	%Bottom row
%	\node[block] (block2_1) [below = 21.25ex of input_int] {$ B $};	% что такое input_int? интересно как это вообще скомпилировалось :)
%	\node[point] (LineSeparator1_1) 	 [left = 2.5ex of block1_1] {};	% здесь и далее всё по аналогии с тем, что я уже говорил
%	\fill (LineSeparator1_1) circle (1pt);
	
%	\node[point] (LineSeparator2_1) 	 [left = 2.5ex of block2_1] {};
%	\draw (LineSeparator1_1) |- (LineSeparator2_1) [vec] to (block2_1);
%	
%	\mysum{right = 1em of block2_1}{sum2_1};
%	\draw[vec] (block2_1) to (sum2_1);
	
%	\node[block] (block2_2)[right = 8em of block2_1] {$\in$};	
%	\draw[vec] (sum2_1) to (block2_2);
	
%	\node[block] (block2_3)[above = 1em of block2_2] {$\left( D-GC \right)\left( A{{L}_{1}}-A{{L}_{2}}G \right)$ };
%	\node[block] (block2_4)[below = 1em of block2_2] {$\left( D-GC \right)A{{L}_{2}}$ };	


%	\node[block](block2_5)[right = 6em of block2_2]{${{L}_{2}}$};
	
%	\draw[vec] (block2_2) to (block2_5);
	
		
%	\node[block](block2_6)[right = 5em of block2_3]{${{L}_{1}}+{{L}_{2}}G$};
%	\mysum{below = 2em of block2_6}{sum2_3};	
%	\draw[vec] (block2_6) to (sum2_3);
%	\draw[vec] (block2_5) to (sum2_3);
	
		
	%and there
%	\node[point] (LineSeparator1_6) [right = 4em of block1_4]{}; 
%	\fill (LineSeparator1_6) circle (1pt);	
%	\draw[vec] (LineSeparator1_6) to (block2_6);
	%and there
		
%	\draw (v) |- (sep) [vec] to (input_int2);
	%\path[draw] node{separator1}  -- (1,0) -- (1,1) node {input_int2};

	
	
%	\draw[vec] (input) to node{$ U $} (input_int);
...
\end{matrix} \right)x+\left( \begin{matrix}
   0  \\
   0.8  \\
\end{matrix} \right)u(t)\] измеряется лишь переменная $y={{x}_{1}}\,,\,c\,=(1,0)$. Построим наблюдатель полного (2) порядка, восстанавливающий оценки ${{\bar{x}}_{1}},{{\bar{x}}_{2}}.$ Матрица наблюдателя:
 \[L={{({{l}_{1}},{{l}_{2}})}^{T}}\] 
 \[(A-LC)=\left( \begin{matrix}
   -{{l}_{1}} & 1  \\
   -{{l}_{2}} & -5  \\
\end{matrix} \right)\] 
Полоса наблюдателя определяется уравнением:
\[\det (\lambda E-A+LC)=\det \left| \begin{matrix}
   \lambda +{{l}_{1}} & -1  \\
   {{l}_{2}} & \lambda +5  \\
\end{matrix} \right|
\\=(\lambda +{{l}_{1}})(\lambda +5)+{{l}_{2}}=\]
\[={{\lambda }^{2}}+(5+{{l}_{1}})\lambda +(5{{l}_{1}}+{{l}_{2}})=0\]
Поскольку характеристические числа системы $\left\{ 0;-5 \right\}$, зададим полюса наблюдателя $\left\{ -20\pm j10 \right\}$, т.е. характеристический полином наблюдателя имеет вид:
\[\begin{array}{*{35}{l}}
   {{\lambda }^{2}}+40\lambda +500={{\lambda }^{2}}+2\alpha \lambda +{{\alpha }^{2}}+{{\beta }^{2}}  \\
   {{l}_{1}}=35  \\
   {{l}_{2}}=500-5\cdot35=325  \\
\end{array},\]
а модель ассимптотического наблюдателя имеет вид:
\[\begin{array}{*{35}{l}}
   \dot{\bar{x}}=(A-LC)\bar{x}+Bu+Ly=\left[ \left( \begin{matrix}
   0 & 1  \\
   0 & -5  \\
\end{matrix} \right)-\left( \begin{matrix}
   0  \\
...
\begin{tikzpicture}	
	...
	\node[point] (inp2) [above = 2ex of block1_2] {${{x}_{2}}\left( 0 \right)$};
	\draw[vec] (inp2) to (block1_2);
	...
	\node[block] (block1_3)[below = of block1_2] {$5$};
	
	\node[block] (block1_4)[right = 20ex of block1_2] {$1/p$};

	\node[point] (inp3) [above = 2ex of block1_4] {${{x}_{1}}\left( 0 \right)$};
	\draw[vec] (inp3) to (block1_4);
	
	
	%\node[point] (LineSeparator1_2) [right = 7.5ex of block1_2]{};
	
	\draw[vec] (block1_2) to node(LineSeparator1_2){${{x}_{2}}$} (block1_4);
	...
	%%Вторая линия
	\node[block] (block2_1)[below =30ex of block1_1] {$0.8$};
	\mysum{right = 8ex of block2_1}{sum2_1};
	
	
	\node[block] (block2_2)[right = 13ex of sum2_1] {$1/p$};
	\node[block] (block2_3)[above = of sum2_1] {$325$};
	\node[block] (block2_4)[below = of block2_2] {$5$};
	\node[point] (LineSeparator1_1) [left = 3.5ex of block1_1] {};
	\fill (LineSeparator1_1) circle (2pt);	
	\draw[vec] (LineSeparator1_1) |- (block2_1);	
	
	
	\mysum{right = of block2_2}{sum2_3};
	\node[block] (block2_6)[above = of sum2_3] {$35$};
	
	\node[block] (block2_5)[right = 12ex of sum2_3] {$1/p$};
	...
	\draw[vec] (block2_2) to (sum2_3);	

	
	\draw[vec] (sum2_3) to (block2_5);
	
	\draw[vec] (block2_3) to (sum2_1);
	...
	\node[point] (LineSeparator2_1) [right = 15ex of block2_2]{};
	\node[point] (LineSeparator2_10) [above = 1ex of LineSeparator2_1]{$z$};
	\fill (LineSeparator2_1) circle (2pt);
	...
	\node[point] (LineSeparator2_20) [below =18ex of LineSeparator1_3]{};
%	\mysum{below =18ex of LineSeparator1_3}{sum2_4};	
	
	
	%\draw[vec] (LineSeparator1_3) to (sum2_4);	

	\draw[vec] (LineSeparator1_3) to (LineSeparator2_20)-| (block2_6);	
	\draw[vec] (LineSeparator1_3) to (LineSeparator2_20)-| (block2_3);	
...
%	\draw[vec] (block2_5) -| (sum2_4);
	
	
	\node[point] (out10)[right = 5ex of  block2_5]{};
	\draw[vec] (block2_5) to node(out50){${{\overline{x}}_{1}}$} (out10);
	\fill (out50.south) circle (2pt);
	
	\mysum{right = 3ex of block2_1}{sum2_10};
	\node[block] (block2_50)[below = 15ex of sum2_10] {$325$};
	
	\node[block] (block2_51)[below = 15ex of sum2_3] {$35$}; 
	
	\draw[vec] (block2_1) to (sum2_10);	
	\draw[vec] (sum2_10) to (sum2_1);	
	
	
	\node[point] (block2_52)[below = 25ex of  out50]{};
	
	\node[point] (block2_53)[below = 3ex of  block2_50]{};
	\node[point] (block2_54)[below = 3ex of  block2_51]{};
	
	\draw[vec] (out50) to (block2_52)-| (block2_53) to (block2_50);	
	
	\draw[vec] (block2_54) to (block2_51);	
	
	
	\draw[vec] (block2_50) to (sum2_10);	
	
	\draw[vec] (block2_51) to (sum2_3);	
	
	%\node[point] (out10)[right = 2ex of  block2_5]{${{\overline{x}}_{1}}$ };	
	
%	\node[point] (out11)[right = 35ex of  block2_4]{};
%	\draw[vec] (block2_4) to (out11);
	
	
	
\end{tikzpicture}
\footnotesize \caption{asdsadasdsadsadsd\label{fig:extremum:fourth_example}}
\end{figure}
...
\begin{tikzpicture}	
	...
	\draw[vec] (sum2_4)-| (block2_6);	
	\draw[vec] (sum2_4)-| (block2_3);	
...
\end{tikzpicture}
\footnotesize \caption{Рыфвфывфывыфве\label{fig:extremum:fourth_example}}
\end{figure}


\begin{problem}
Синтезируем наблюдатель Люенбергера 1 порядка. Для этого выберем матрицу $D$ так, чтобы матрица $\left| \begin{matrix}
   C  \\
   D  \\
...
\end{matrix} \right|,{{L}_{2}}=\left| \begin{matrix}
   0  \\
   1  \\
\end{matrix} \right|$. Матрица $G$ - скаляр равный $g$, $(D-GC)=(-g,1).$
...
\end{matrix} \right)\left( \begin{matrix}
   0  \\
   1  \\
\end{matrix} \right)z+\bigg(\left( \begin{matrix}
   0 & 1  \\
   0 & -5  \\
\end{matrix} \right)\left( \begin{matrix}
   1  \\
...
\end{matrix} \right)\left( \begin{matrix}
   0  \\
   1  \\
\end{matrix} \bigg) \right)y+
\left( \begin{matrix}
   0  \\
   0.8  \\
\end{matrix} \right)u \bigg];\\
\end{array}
 \\
   z(0)=0;  \\
...
\[
\dot{z}=\left( -g,1 \right)\left[ \left( \begin{matrix}
   1  \\
   -5  \\
\end{matrix} \right)z+\left( \begin{matrix}
   1  \\
...
\end{matrix} \right)u 
\right]
=-\left( g+5 \right)z-\left( g+5 \right);\]



Полюс наблюдателя равен $-(g+5)$. Выберем в точке $-20$, тогда $g$ равно 15.
И окончательно получим уравнение: 
\[\dot{z}=-20z-300y+0.8u,\,\,z(0)=0,{{\bar{x}}_{1}}=y,{{\bar{x}}_{2}}=z+15y\]
Cтруктура имеет вид:
...
\begin{tikzpicture}	
	...
	\node[block] (block2_2)[right = 13ex of sum2_1] {$1/p$};
	\node[block] (block2_3)[above = of block2_2] {$300$};
	\node[block] (block2_4)[below = of block2_2] {$20$};
	\node[point] (LineSeparator1_1) [left = 3.5ex of block1_1] {};
	\fill (LineSeparator1_1) circle (2pt);	
	\draw[vec] (LineSeparator1_1) |- (block2_1);	
	...
	\node[block] (block2_6)[below = 20ex of LineSeparator1_3] {$15$};
	\draw[vec] (LineSeparator2_1) |- (block2_4);	
	

		
	\mysum{below =9ex of block2_6}{sum2_2};	
	...
	\node[point] (output2) [right = 2ex of sum2_2] {${{\overline{x}}_{2}}$};	
	...
	\node[point] (LineSeparator1_5) [right = 15ex of block2_3] {};
